...
import { fetchReviewsByTripId, fetchAgencyById, ReviewData } from "@/lib/firebase/firestore";
...
export default function DetailPage() {
  ...
  useEffect(() => {
    if (!firebaseReady || !t.id) return;
    fetchReviewsByTripId(t.id).then(setReviews).catch(() => {});
    if (t.agencyId) {
      fetchAgencyById(t.agencyId).then((agency) => {
        if (agency) setAgencyPhone((agency as any).phone || "");
      }).catch(() => {});
    }
  }, [firebaseReady, t.id, t.agencyId]);
        ...
        <div className="card mb-5 overflow-hidden">
          <div className="px-6 py-5 bg-gradient-to-r from-navy to-[#1a2f55] text-white">
            <h3 className="text-[17px] font-bold">
              📅 {tripDate || "Date non renseignée"} · {t.agency}
            </h3>
          </div>
          <div className="p-6">
        ...
        <div className="card mb-5 overflow-hidden">
          ...
          <div className="p-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <InfoItem icon={<Bus size={20} />} label="Type de véhicule" value={`Bus climatisé ${t.total || 32} places`} />
              ...
              <InfoItem icon={<Snowflake size={20} />} label="Climatisation" value={t.amenities.includes("❄️") ? "Oui, incluse" : "Non spécifié"} />
              <InfoItem icon={<Luggage size={20} />} label="Bagages" value={t.amenities.includes("🧳") ? "1 bagage en soute inclus" : "Non spécifié"} />
            </div>
          ...
          <div className="p-6 flex gap-4 items-start">
            ...
            <div>
              ...
              <div className="flex gap-4 flex-wrap text-[13px] text-greyMid">
                <span>📞 {agencyPhone || "+237 XXX XXX XXX"}</span>
                <span>📍 {t.depart}, Cameroun</span>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="p-6">
            ...
            <div className="flex flex-col gap-4">
              {reviews.length > 0 ? reviews.map((r, i) => (
                <div key={r.id || i} className="p-4 bg-bg rounded-lg">
                ...
                <p className="text-sm text-greyMid">Aucun avis pour le moment.</p>
              )}
            </div>
          </div>
        </div>
